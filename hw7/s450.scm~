...
(define (xeval exp env)
  ...
  (cond ((self-evaluating? exp) exp)
        ...
        ;;Checks if exp is the eof-object? and if so, we return cleanly to UMB
        ;;scheme.
        ((eof-object? exp)
         (eval-exit))
        ...
        ((AND(OR (definition? exp) (assignment? exp))
         ...
         (s450error "Cannot redefine/assign special form -- " exp))
...
;;Primitive procedure to cleanly exit scheme
(define (eval-exit)
  (newline)
  (UMB-SCHEME "Exited from (s450)")
  )
    ...
    (define (scan vars vals)
      (cond ((null? vars)
            ...
            ((AND (eq? var (car vars))
                  (pair? (car vals))
                  (eq? (caar vals) 'delay))
             (xeval (car (cdr (car vals)))
                    (car (cdr (cdr (car vals))))))
            ((eq? var (car vars))
             (car vals))
...
;;Modified the (s450) to place a call/cc procedure there. If (goto-s450) is
;;used, then the user returns to the (s450) prompt.
(define (s450)
  (call/cc
   (lambda (here)
     (set! goto-s450 here)))
...
;;Modified the start message, so if the user wants to exit (s450) then it
;;returns the user to the UMB Scheme environment that the orginial message came
;;from.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; Call/cc and s450error
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;Variable to hold (s450) goto
(define goto-s450 '())

;;s45error takes a variable number of arguments, then prints them. Once done
;;displaying the procedure returns to s450.
...
(install-primitive-procedure '/ /)
(install-primitive-procedure 'exit eval-exit)
