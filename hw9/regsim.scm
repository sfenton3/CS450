...
(define (make-new-machine)
    (let ((pc (make-register))
          (flag (make-register))
          ;;Register to hold breakpoints
          (bk (make-register))
          ;;Register to hold labels
          (lb (make-register))
          (stack (make-stack))
      ...
      (let ((the-ops
        ...
        (define (execute)
          (let ((insts (get-contents pc))
                (breakpoints (create-breakpoints lab-list (get-contents bk) )))

            (cond
              ((null? insts) 'done)
              ;;Checks to see if the current instruction number corresponds to a
              ;;breakpoint instruction number. The continue predicate stops an
              ;;infinite loop of breaking when we continue the machine again.
              ((AND (is-bk? breakpoints (search-list instr-list (caar insts)))
                    continue?)
               (set! continue? #f)
               ;;Print label and transform back to original
               (display "Breakpoint at Label: ")
               ;;Finds the breakpoint label
               (display (bk-val breakpoints
                                (search-list instr-list (caar insts))))
               (display "  position: ")
               ;;Translates back to the original breakpoint value
               (display
                (- (+ 1 (search-list instr-list (caar insts)))
                   (search-labels lab-list
                                  (bk-val breakpoints
                                          (search-list instr-list
                                                       (caar insts))))))
               (newline))
              ;;Checks special case if breakpoint is at last line.
              ((AND (is-bk? breakpoints
                            (+ 1 (search-list instr-list (caar insts))))
                    (equal? (search-list instr-list (caar insts))
                            (search-list-max instr-list (caar insts)))
                    continue? )
               (set! continue? #f)
               ;;Print label and transform back to original
               (display "Breakpoint at Label: ")
               ;;Finds the breakpoint label
               (display (bk-val breakpoints
                                (+ (search-list instr-list (caar insts)) 1)))
               (display "  position: ")
               ;;Translates back to the original breakpoint value
               (display (- (+ 2 (search-list instr-list (caar insts)))
                           (search-labels
                            lab-list
                            (bk-val breakpoints
                                    (+ 1
                                       (search-list instr-list
                                                    (caar insts)))))))
                              (newline))
              ;;We set the continue? predicate back to true, so we can catch
              ;;breakpoints again.
              (else
               (set! continue? #t)
        ...
        (define (dispatch message)
          (cond ((eq? message 'start)
                 ...
                 (set! instr-list (dit the-instruction-sequence))
                 (set! lab-list (lb-list (get-contents lb) instr-list))
                 (execute))
                ((eq? message 'proceed)
                 (execute))
                ((eq? message 'lb)
...
;;Returns true if the current instruction value is equal to the value of any of
;;the current breakpoint values
(define (is-bk? seq val)
  (cond
    ((null? seq) #f)
    ((equal? (car (cdr (car seq))) val)
     #t)
    (else
     (is-bk? (cdr seq) val))))

;;Returns the label of the breakpoint
(define (bk-val seq val)
  (cond
    ((null? seq) '())
    ((equal? (car (cdr (car seq))) val)
     (caar seq))
...
;;Returns a list of breakpoints shifted to the true value
(define (create-breakpoints labels breakpoints)

  (define (search-labels seq val)
    (cond
      ((null? seq) -1)
      ((equal? (caar seq) val)
       (car (cdr (car seq))))
      (else
       (search-labels (cdr seq) val))))

  (define (combine-labels bk)
    (cond
      ((null? bk) '())
      (else
       (append (list (list (caar bk)
                           (+ (search-labels labels (caar bk))
                              (- (car (cdr (car bk))) 1) )))
               (combine-labels (cdr bk))))))

  (cond
    ((null? breakpoints) '())
    ((equal? breakpoints '*unassigned*) '())
    (else
     (combine-labels breakpoints)))
     
  )


;;Returns a list of all the current label positions. 
(define (lb-list seq oldseq)
  
  (define (append-val seq)
    (cond
     ((AND (null? (cdr seq)) (null?  (cdr (car seq))))
      (list (list (caar seq)
                  (+ (search-list-max oldseq 0) 1))))
     ((null? (cdr seq))
      (list (list (caar seq)
                  (search-list oldseq
                               (car (car
  ...
  (cond
   ((OR (null? seq) (equal? seq '*unassigned*))
    '())
   (else
    (append-val seq))))

;;Returns a list of all the instructions in the machine, with a number
;;corresponding to there order of execution.
(define (dit seq)
    
  (define (append-val seq counter)
    (cond
      ((null? seq) '())
      (else
       (cons (list (caar seq) counter)
             (append-val (cdr seq) (+ counter 1))))))

   (append-val seq 1))

;;Looks through the list until it finds the instruction with the matching value,
;;and returns the order of executuion
(define (search-list seq val)
  ...
  (cond
    ...
    ((equal? (caar seq) val)
     (car (cdr (car seq))))
    (else
     (search-list (cdr seq) val)))
     )

(define (search-reg seq val)
  (cond
    ((null? seq) #f)
    ((equal? (caar seq) val)
     #t)
    (else
     (search-reg (cdr seq) val)))
     )

;;Retuns the max value of the instr-list
(define (search-list-max seq val)  
  (cond
    ((null? seq) val)
    (else
     (search-list-max (cdr seq) (car (cdr (car seq))))))
     )
...
;;Instruction to proceed with machine
(define (proceed-machine machine)
  (machine 'proceed))

;;Data structure to keep a list of breakpoints
(define (set-breakpoint machine label n)
  (let ((val (append (list label) (list n)))
        (var (get-register-contents machine 'bk)))
    (cond
      ((OR (equal? var '*unassigned*) (null? var))
      (set-register-contents! machine 'bk (list val)))
      (else
       (set-register-contents! machine 'bk
                               (append var (list val)))))))

;;Instruction to clear all breakpoints in bk register
(define (cancel-all-breakpoints machine)
  (set-register-contents! machine 'bk '()))

;;Instruction to remove specific breakpoint from bk register
(define (cancel-breakpoint machine label n)
  (let ((val (get-register-contents machine 'bk))
        (var (append (list label) (list n)))
        (labels (lab machine)))


    ;;The instruction value at position n
    (define (offset var n)
      (+ (- (search-labels labels (car var)) 1) n))

    ;;Removes breakpoint if the offsets are the same
    (define (remove-val var val)
      (cond
        ((null? val) '())
        ((equal? (offset var n) (offset (car val) (car (cdr (car val)))))
         (remove-val var (cdr val)))
        (else
         (cons (car val) (remove-val var (cdr val))))))

    (cond
      ((null? (remove-val var val))
       (set-register-contents! machine 'bk '()))
      (else
       (set-register-contents! machine 'bk
                               (remove-val var val))))))
...
(define (assemble controller-text machine)
  (extract-labels controller-text
                  (lambda (insts labels)
                    ...
                    (set-register-contents! machine 'lb labels)
                    (set! label-name '())
                    insts)))
       ...
       (lambda (insts labels)             ;; This is where labels and insts are
         (let ((next-inst (car text)))    ;;   accumulated (as on a stack):
           (cond ((symbol? next-inst)                 
                  (if (search-reg label-name next-inst)
                      (error "Cannot have two labels with the same name")
                      (set! label-name
                            (append label-name (list (list next-inst)))))
                  (receive insts                              ;; either like
                                                              ;; this
...
(define (make-elementary-exp exp machine labels)
  (cond ((constant-exp? exp)
         (let ((c (constant-exp-value exp)))
           (lambda () c)))
        ((AND (label-exp? exp) (list? (cadr exp)))
         (lambda () (lookup-label
                     labels
